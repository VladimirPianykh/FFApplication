...
public class PreparationTaskManager {
    ...
    static {
        tasks = new ArrayList<>();
        loadTasks(); // Загружаем задачи из файла при создании экземпляра
    }

    // Метод возвращает список задач
    public static List<PreparationTask> getTasks() {
        return tasks;
    }

    // Метод для регистрации новой задачи
    public static void registerTask(PreparationTask task) {
        tasks.add(task);
    }
    ...
    private static void loadTasks() {
        ...
        if (file.exists()) {
            try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream(file))) {
                tasks = (ArrayList<PreparationTask>) ois.readObject(); // Десериализация списка задач
